...
public class DragAndDrop : MonoBehaviour
{
    ...
    private void MousePressed(InputAction.CallbackContext context)
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            {
                Vector3 mousePositionInWorld = mainCamera.ScreenToWorldPoint(Mouse.current.position.ReadValue());
                GameObject clickedObject = hit.collider.gameObject;
                Vector2 in2dSpace = new Vector2(mousePositionInWorld.x - clickedObject.transform.position.x , mousePositionInWorld.z - clickedObject.transform.position.z );
                Debug.Log(in2dSpace);
