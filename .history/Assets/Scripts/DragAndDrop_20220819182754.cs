...
public class DragAndDrop : MonoBehaviour
{
    ...
    private bool willStackWithAnotherCard(GameObject clickedObject)
    {
        ...
        {
            Debug.Log(`${leftTopCornerHit}`);
        }
        RaycastHit rightTopCornerHit;
        if (Physics.Raycast(leftTopCornerPoint, transform.TransformDirection(Vector3.down), out rightTopCornerHit, 20, cardLayerMask))
        {
            Debug.Log("Did Hit");
        }
        RaycastHit leftBottomCornerHit;
        if (Physics.Raycast(leftTopCornerPoint, transform.TransformDirection(Vector3.down), out leftBottomCornerHit, 20, cardLayerMask))
        {
            Debug.Log("Did Hit");
        }
        RaycastHit rightBotttomCornerHit;
        if (Physics.Raycast(leftTopCornerPoint, transform.TransformDirection(Vector3.down), out rightBotttomCornerHit, 20, cardLayerMask))
        {
            Debug.Log("Did Hit");
        }
