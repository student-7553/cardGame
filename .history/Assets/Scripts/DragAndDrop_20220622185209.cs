...
public class DragAndDrop : MonoBehaviour
{
    ...
    private IEnumerator DragUpdate(GameObject clickedObject)
    {
        ...
        while (mouseClick.ReadValue<float>() != 0)
        {
            ...
            {
                Vector3 tempo =  ray.GetPoint(initialDistance);
                // clickedObject.transform.position
                clickedObject.transform.position = Vector3.SmoothDamp(clickedObject.transform.position,, ref velocity, mouseDragSpeed);
                yield return null;
            }
        }
    }
